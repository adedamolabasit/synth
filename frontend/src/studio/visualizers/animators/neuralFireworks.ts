  ...
  objects.forEach((obj) => {
    ...
    fireworks.forEach((firework, index) => {
      ...
      const freqIndex = Math.floor(
        (index / fireworks.length) * frequencyData.length
      );
      ...
      for (let i = 0; i < positions.length; i += 3) {
        const explosionPhase = lifeCycle * 3;
        ...
        if (explosionPhase < 1) {
          ...
          positions[i + 1] +=
            velocities[i + 1] * explosionState * (1 + bass * 2);
          positions[i + 2] +=
            velocities[i + 2] * explosionState * (1 + treble * 2);
        } else {
          positions[i] += velocities[i] * 0.1;
          positions[i + 1] += velocities[i + 1] * 0.1 - 0.02;
        ...
        const brightness =
          Math.max(0.3, 1 - explosionPhase * 0.5) + energy * 0.3;
    ...
    obj.userData.neuralConnections.forEach(
      (connection: THREE.Line, index: number) => {
    ...
    if (beatInfo?.isBeat) {
      fireworks.forEach((_firework, index) => {
        if (Math.random() < 0.6) {
          lifeCycles[index] = 0.1;
