...
export const animateNeuralCosmos = (
  ...
  // Cinematic neural network exploration
  if (camera) {
    // Dynamic camera that explores the neural network
    const neuralOrbit = time * 0.03;
    const neuralRadius = 18 + Math.sin(time * 0.08) * 4;
    const neuralHeight = Math.sin(time * 0.05) * 8;
    
    // Smooth camera movement with occasional neural jumps
    ...
    // Cinematic camera shake on intense beats
  ...
  objects.forEach((obj) => {
    ...
    const neuralIntensity = params.intensity * 0.035;
    ...
    neurons.forEach((neuron, index) => {
      ...
      const freqIndex = Math.floor((index / neurons.length) * frequencyData.length);
      ...
      // Dynamic material based on activation
    ...
    synapticFields.forEach((field, index) => {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        positions[i + 1] = Math.sin(angle * 1.3) * (1.5 + Math.cos(time * 1.7) * 0.5);
        positions[i + 2] = Math.sin(angle) * (1.5 + Math.cos(time * 1.5) * 0.5);

        // Synaptic color based on activation
        const hue = (index * 0.005 + activation * 0.3) % 1;
        const color = new THREE.Color().setHSL(hue, 0.8, 0.6 + activation * 0.3);
        colors[i] = color.r;
        colors[i + 1] = color.g;
        colors[i + 2] = color.b;
      }
    ...
    // Animate axons (neural connections)
    ...
    for (let i = 0; i < impulseProgress.length; i++) {
      ...
      const startIdx = impulsePaths[i3];
      const endIdx = impulsePaths[i3 + 1];
      const speed = impulsePaths[i3 + 2] * 0.02 + 0.01;

      // Move impulse along neural path
      progress = (progress + speed * (1 + bass)) % 1;
      impulseProgress[i] = progress;

      if (progress < 0.01) {
        // Reset impulse with new random path
        const newStart = Math.floor(Math.random() * neurons.length);
        const newEnd = (newStart + Math.floor(Math.random() * 20) + 1) % neurons.length;
      ...
      impulsePositions[i3 + 1] = startPos.y + (endPos.y - startPos.y) * progress;
      impulsePositions[i3 + 2] = startPos.z + (endPos.z - startPos.z) * progress;
    }

    impulseGeometry.attributes.position.needsUpdate = true;

    // Global neural network rotation
    obj.rotation.y += cosmicSpeed * bass * 0.3;
    obj.rotation.x += cosmicSpeed * mid * 0.2;

    // Neural cascade on beat
    if (beatInfo?.isBeat) {
