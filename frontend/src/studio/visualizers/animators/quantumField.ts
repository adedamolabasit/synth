  ...
  objects.forEach((obj) => {
    ...
    for (let i = 0; i < obj.userData.particleCount; i++) {
      ...
      const freqIndex = Math.floor(
        (i / obj.userData.particleCount) * frequencyData.length
      );
      const energy = frequencyData[freqIndex] / 255;

      const waveFunction =
        Math.sin(time * 2 + quantumStates[i]) * energy * intensity;
      const probabilityDensity =
        Math.sin(time * 3 + quantumStates[i] * 2) * 0.5 + 0.5;

      positions[i3] =
        originalPositions[i3] +
        Math.sin(time + quantumStates[i]) * waveFunction;
      positions[i3 + 1] =
        originalPositions[i3 + 1] +
        Math.cos(time * 1.3 + quantumStates[i]) * waveFunction;
      positions[i3 + 2] =
        originalPositions[i3 + 2] +
        Math.sin(time * 0.7 + quantumStates[i]) * waveFunction;
      ...
      const hue =
        (i / obj.userData.particleCount + time * 0.1 + energy * 0.2) % 1;
