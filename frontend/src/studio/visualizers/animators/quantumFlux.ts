...
export const animateQuantumFlux = (
  ...
  // Quantum field camera - exploring the flux
  if (camera) {
  ...
  objects.forEach((obj) => {
    ...
    const quantumIntensity = params.intensity * 0.03;
    ...
    fluxTubes.forEach((tube, index) => {
      ...
      const freqIndex = Math.floor((index / fluxTubes.length) * frequencyData.length);
    ...
    probabilityWaves.forEach((wave, index) => {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        // Orbital motion around tube
        const orbitRadius = 2 + Math.sin(time * 2 + phase + particleIndex * 0.1) * 1;
    ...
    if (beatInfo?.isBeat) {
      ...
      // Probability wave intensification
