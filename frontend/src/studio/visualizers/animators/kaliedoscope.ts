...
export const animateKaleidoscope = (
  ...
  objects.forEach((obj) => {
    if (obj instanceof THREE.Mesh) {
      ...
      const angle =
        (segment / 8) * Math.PI * 2 + scaledTime * spinSpeed + rotationOffset;
      const layerRadius =
        radius + Math.sin(scaledTime * 2 + layer) * 0.3 * audioValue;
      ...
      obj.position.z =
        (layer - 2) * 0.3 +
        Math.sin(scaledTime * 3 + segment) * 0.15 * audioValue;

      const scale =
        0.8 + audioValue * 0.8 + Math.sin(scaledTime * 4 + segment) * 0.2;
      obj.scale.set(scale, scale, scale);

      if (obj.material instanceof THREE.MeshPhongMaterial) {
        const hue =
          (segment / 8 + layer * 0.1 + scaledTime * 0.05 + audioValue * 0.3) %
          1;
