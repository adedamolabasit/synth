...
// import { beatPulse, animateCamera, rotateScene } from "../effects/gsapAnimation";
import { beatPulse, animateCamera, rotateScene } from "../../effects/gsapAnimations";
...
export const animateBiomorphic = (
  ...
  // Animate camera using GSAP if available
  ...
  objects.forEach((obj) => {
    obj.traverse((child) => {
      if (!(child instanceof THREE.Mesh) || child.userData.depth === undefined) return;
      ...
      const waveY = Math.sin(scaledTime * 1.5 + depth * 1.2 + pulsePhase) * 0.05 * (1 + bass);
      const waveZ = Math.cos(scaledTime * 1.7 + depth * 0.8) * 0.05 * (1 + treble);
      ...
      child.rotation.x += Math.sin(scaledTime * 1.5 + depth) * 0.02 * audioValue;
      child.rotation.y += Math.cos(scaledTime * 1.3 + depth) * 0.02 * audioValue;
      child.rotation.z += Math.sin(scaledTime * 1.7 + depth) * 0.01 * audioValue;

      if (child.material instanceof THREE.MeshPhongMaterial) {
        const { hue, lightness } = colorShift(scaledTime, depth, audioValue, beatInfo);
  ...
  // Global wave motion on parent objects
 ...
 if (scene) {
  const hue = (scaledTime * 0.2 + bass * 0.3) % 1;  // faster hue shift
