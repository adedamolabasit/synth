...
export const createLiquidVisualizer = (scene: THREE.Scene, params: VisualizerParams): THREE.Object3D[] => {
  ...
  liquidSurface.userData = {
    ...
    waveCenters: Array.from({ length: 8 }, () => ({
      x: Math.random() * 10 - 5,
      y: Math.random() * 10 - 5,
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const color = new THREE.Color().setHSL(Math.random() * 0.15 + 0.55, 0.7 + Math.random() * 0.3, 0.4 + Math.random() * 0.3);
    colors[i3] = color.r;
    colors[i3 + 1] = color.g;
    colors[i3 + 2] = color.b;
  ...
  const particleMaterial = new THREE.PointsMaterial({ size: 0.1, vertexColors: true, transparent: true, sizeAttenuation: true });
  ...
  const lineMaterial = new THREE.LineBasicMaterial({ color: new THREE.Color().setHSL(0.6, 0.5, 0.4), transparent: true, opacity: 0.3 });
