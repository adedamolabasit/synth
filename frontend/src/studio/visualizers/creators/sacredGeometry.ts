...
export const createSacredGeometryVisualizer = (scene: THREE.Scene, params: VisualizerParams): THREE.Object3D[] => {
  ...
  for (let layer = 0; layer < layers; layer++) {
    ...
    for (let i = 0; i < circles; i++) {
      ...
      torus.position.set(
        ...
        layer * 0.2 - layers * 0.1
      );
      torus.userData = { layer, angle, index: i };
      scene.add(torus);
      objects.push(torus);
    }
  }
  ...
  vertices.forEach((vertex, i) => {
    ...
    sphere.userData = { isVertex: true, index: i };
    scene.add(sphere);
    objects.push(sphere);
  });
