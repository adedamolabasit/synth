...
export class LyricsManager {
  ...
  update(currentTime: number): void {
    ...
    if (currentSegmentIndex >= 0) {
      ...
      if (currentSegment.words && this.config.showWordHighlight) {
        const currentWordIndex = this.findCurrentWordIndex(
        ...
        this.currentState.currentWord =
          currentWordIndex >= 0
            ? currentSegment.words[currentWordIndex].word
            : "";
      } else {
      ...
      const segmentProgress =
        segmentDuration > 0
          ? (currentTime - currentSegment.start) / segmentDuration
          : 0;
  ...
  getFormattedLine(): { text: string; words: WordTimestamp[] } | null {
    ...
    const segment =
      this.lyricsData.segments[this.currentState.currentLineIndex];
    return {
      text: segment.text,
      words: segment.words || [],
    };
