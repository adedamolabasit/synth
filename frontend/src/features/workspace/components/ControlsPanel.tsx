...
export const useThreeSetup = () => {
  ...
  const calculateCameraPosition = useCallback((): [number, number, number] => {
    const angleRad = (params.cameraAngle * Math.PI) / 180;
    const elevationRad = (params.cameraElevation * Math.PI) / 180;
    
    const x = params.cameraDistance * Math.cos(elevationRad) * Math.sin(angleRad);
    const y = params.cameraDistance * Math.sin(elevationRad);
    const z = params.cameraDistance * Math.cos(elevationRad) * Math.cos(angleRad);
    
    return [x, y, z];
  }, [params.cameraAngle, params.cameraElevation, params.cameraDistance]);
  ...
  const animateScene = useCallback(
    (time: number) => {
      ...
      if (params.rotationSpeed === 0) {
        ...
        const cameraDistance = params.cameraDistance + bass * 5;
        const cameraSpeed = params.rotationSpeed * 0.005 + bass * 0.01;
        const angleRad = (params.cameraAngle * Math.PI) / 180;
        const elevationRad = (params.cameraElevation * Math.PI) / 180;

        cameraRef.current.position.x = 
          cameraDistance * Math.cos(elevationRad) * Math.sin(currentTime * cameraSpeed + angleRad);
        cameraRef.current.position.y = 
          cameraDistance * Math.sin(elevationRad);
        cameraRef.current.position.z = 
          cameraDistance * Math.cos(elevationRad) * Math.cos(currentTime * cameraSpeed + angleRad);
        cameraRef.current.lookAt(0, 0, 0);
      }
